...
class SellMax(StrategyAbstract):
    def __init__(self, acting_frequency, buy_and_sell_manager):
        super().__init__(None, acting_frequency)
    ...
    @staticmethod
    def sell_gold(_params, prev_state):
    ...
    def execute_optimal_action(self, params, prev_state):
        ...
        if prev_state["timestep"] % self.acting_frequency != 0:
            ...
            return self.buy_and_sell_manager.leave_all_state_variables_unchanged(
                prev_state=prev_state, policy_type="exchange"
            )

        self.optimize(params, prev_state)
        buy_amount = (
            self.buy_and_sell_manager.calculate_buy_amount_constant_product_amm(
                params=params,
